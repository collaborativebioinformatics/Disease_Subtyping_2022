...
To confirm the assertion made by a number of authors (REFS) that the efficacy of treatment varies based on subtypes of patients, we have performed survival analysis on the colorectal cancer patients stratified by subtypes and treatment groups, which is seen in our script found in the 'scripts/' folder. This function allows the user to input a subtype and treatment groups that you want to compare, then it returns overall survival of such patient with a certain subtype and treatments. This analysis showed that CMS4 patients who received bevacizumab, fluorouracil, leucovorin, and irinotecan survied longer than those who received only fluorouracil, leucovorin, and irinotecan. On the other hand, CMS1 patients who received bevacizumab, fluorouracil, leucovorin, and irinotecan survived shorter than those who received only fluorouracil, leucovorin, and irinotecan. This result provides further support for the previous assertion that subtyping can increase the efficacy of treatments. This survival analysis platform can be used for users to determine which treatment is best for a particular subtype of patient for commonly used treatments.  
...
**Figure 1: survival curve of patients who received bevacizumab, fluorouracil, leucovorin, and irinotecan vs fluorouracil, leucovorin, and irinotecan (CMS4)**
...
**Figure 1: survival curve of patients who received bevacizumab, fluorouracil, leucovorin, and irinotecan vs fluorouracil, leucovorin, and irinotecan (CMS1)**

To further refine this approach to precision medicine, we then built out a method to to perform RNA-seq or microarray analysis on cancer data, in this colorectal, searching for differentially over- or underexpressed genes that the have been demonstrated in the paper by <colotype paper first author>, et al. [^1] to be associated with different subtypes of colorectal cancer.  Our method also analyzes 12 different reported driver genes (ref 8,9,10) for colorectal cancer (table 1), and associates them with various subtypes (Figure 2 a-d).  For the purposes of this manuscript, we show the top ten driver (or combination there of) subtype combinations, outlined in the radar plots shown in figure 2.  

![Figure 2a](https://github.com/collaborativebioinformatics/Disease_Subtyping_2022/radar_plots/cms1_radar_plot.png)
...
Third, developing both a proteomic and DNA-based method for CMS subtyping that can be leveraged as clinical diagnostics.  Fourth, expanding this analysis to other cancer types that have identified CMS, such as ovarian, bladder and small cell lung cancer.  
