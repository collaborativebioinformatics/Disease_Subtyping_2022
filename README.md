...
To confirm the assertion made by a number of authors that the efficacy of treatment varies based on subtypes of patients [^17][^18][^19], we have performed survival analysis on the colorectal patients stratified by subtypes and treatment groups. First, subtyping of colon cancer patient data was performed using the CMScaller algorithm [^3]. This algorithm uses Nearest Template Prediction [^6] to classify colon cancer expression profiles into predefined subtypes. Subtyped patients were then stratified by treatment groups and CMS subtypes for survival analysis. Data preprocessing, algorithm application and survival analysis code can be found on our github page in the "cmscaller_pipeline" folder and "scripts" folder. The survival analysis function allows the user to input subtype and treatment group(s) for comparison, and returns the overall survival of such patient with that specific subtype and treatment. This analysis showed that CMS4 patients that received bevacizumab, fluorouracil, leucovorin, and irinotecan survied longer than those that received only fluorouracil, leucovorin, and irinotecan. On the other hand, CMS1 patients that received bevacizumab, fluorouracil, leucovorin, and irinotecan survived shorter than those that received only fluorouracil, leucovorin, and irinotecan. This result provides further support for the previous assertion that subtyping can increase the efficacy of treatments. This survival analysis platform can be used to determine which commonly used treatments are best for a particular subtype of colon cancer patients.  
...
To further refine this approach to precision medicine, we then built out a method to perform RNA-seq or microarray analysis on colon cancer data searching for differentially over- or underexpressed genes that have been demonstrated in the paper by <colotype paper first author>, et al [^1] to be associated with different subtypes of colorectal cancer.  Our method also analyzes 12 different reported driver genes [^20] for colorectal cancer (table 1), and associates them with various subtypes (Figure 1 a-d).  For the purposes of this manuscript, we show the top ten driver (or combination there of) subtype combinations, outlined in the radar plots shown in figure 2.  
...
An immediate next step for this project is to combine our pipeline with the observational multidrug pipeline in a subtype dependent way.  We believe this will give the oncology research community the best suggestions for precision delivery of drug combinations.  
